...
const Calendar = () => {
  ...
  const handlePreviousMonth = () => {
    ...
    if (
      currentDayx.getDate() !== currentDay &&
      currentDayx.getMonth() !== selectedDate.getMonth()
    ) {
  ...
  return (
    <View className="h-auto w-full max-w-5xl overflow-hidden rounded bg-red-500">
      ...
      <View className="bg-pink-300">
        <View className="flex flex-row items-center justify-between p-4">
          {daysOfWeek.map((day) => (
            <View key={day} className="border-1 flex flex-1 items-center">
              <Text>{day}</Text>
            </View>
          ))}
        </View>
      ...
      <View className="w-full">
        <View className="grid grid-cols-7 p-4">
          {[...Array(firstDayOfMonth.getDay()).keys()].map((_) => (
            <View
              key={_}
              className={`flex h-4 w-full flex-1 justify-center bg-gray-200 p-8`}
            ></View>
          ))}
          {[...Array(daysInMonth).keys()].map((day) => (
            <View
              key={day + 1}
              className={`flex h-4 flex-1 items-center justify-center bg-white p-8 `}
            >
              <Text
                className={`${
